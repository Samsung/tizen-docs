...
| `http://tizen.org/privilege/dpm.wifi` | partner |  | The application can restrict the use of Wi-Fi networks and Mobile Hotspots. If the phone can't connect to a Wi-Fi network, it may connect to a mobile network. This may result in additional charges depending on user's payment plan.<br>**Allowed to licensed partners.** |
...
| `http://tizen.org/privilege/message.write` | public | Message | The application can write, send, delete, and move text and multimedia messages, download multimedia messages, and change the settings and statuses of messages, such as read or unread. This may result in additional charges depending on user's payment plan. |
...
| `http://tizen.org/privilege/softap` | public |  | The application can manage Soft AP configuration settings, such as the Service Set Identifier (SSID) and password. |
...
| `http://tizen.org/privilege/vpnservice` | public |  | The application can manage the VPN (virtual private network) and change its settings. |
...
| `http://tizen.org/privilege/zigbee.admin` | platform |  | The application can control a connected ZigBee coordinator, e.g. turning it on or off. |
