...
Visuals are the main building block for UI components. Visuals provide reusable rendering logic that is controlled using properties and can be used by all components. Visuals also respond to view size and color changes, and can perform clipping at the renderer level.

Every UI component has 3 states: `NORMAL`, `FOCUSED`, and `DISABLED`. In addition, buttons have 2 substates for each state: `SELECTED` and `UNSELECTED`. A visual must be defined for each state and substate. You can share the same visual across multiple states. The components's current state or substate determines which visuals are shown. For more information on styling and transitioning between state visuals, see [Styling Components with JSON](styling-controls-with-JSON.md).

NUI provides the following visual types:
...
4.  Register the visual.
    ...
    The example visuals in this topic use property registration based on a fixed property index range. The NUI code base has been modified to perform property registration based on automatic index generation. For more information, see [Managing Properties](creating-custom-view-controls.md#properties).

5.  Set the depth index.

    The depth index is the draw order for visuals within a view. Whenever a visual is added, the depth index increases automatically. The most recently-registered visual is always on top.
...
-   You can position and resize visuals within a control using a visual transform property map.
    ...
    The [Tizen.NUI.VisualTransformPropertyType](https://developer.tizen.org/dev-guide/csapi/api/Tizen.NUI.VisualTransformPropertyType.html) enumeration lists the transformation properties that can be defined, such as the origin and anchor point, the offset, and the size:

    -   The origin and anchor points are set to the center of the control by default, or you can specify a different alignment using the [Tizen.NUI.Visual.AlignType](https://developer.tizen.org/dev-guide/csapi/api/Tizen.NUI.Visual.AlignType.html) enumeration values.
    -   The offset and size are defined relative to the control size by default, but can also be defined as a number of pixels by specifying the offset and size policies. For example, if the `OffsetPolicy` value is `[RELATIVE, RELATIVE]` and the `SizePolicy` value is `[ABSOLUTE, ABSOLUTE]`, a visual with an `Offset` value of (0, 0.25) and a `Size` of (20, 20) is positioned 25% above the center of the control and is 20 x 20 pixels in size.
...
The following example shows how to use a `BorderVisual` visual map. The visual is created with the `AddVisual()` method.
...
The following example shows how to use a property map for a color visual. The visual is created with the factory `CreateVisual()` method.
...
| `StopColor`     | `Vector4` array  | Yes             | The color at the stop offsets. At least 2 are required to show a gradient. |
...
The visual type for them all is `Image`.
...
| `DesiredWidth`  | `integer`               | No       | The desired image width. Uses actual image width if not specified. |
| `DesiredHeight` | `integer`               | No       | The desired image height. Uses actual image height if not specified. |
...
The following example shows how to use a property map for a normal image visual. The visual is created with the factory `CreateVisual()` method.
...
The primitive visual renders a simple 3D shape, such as a cube or sphere. The shape is scaled to fit the control. By default, shapes are generated with clockwise winding and back-face culling.
...
| `MixColor`          | `Vector4`           | The shape color.                |
...
| `BevelPercentage`   | `float`             |  For cuboids, the amount of beveling, based on the cuboid's smallest dimension.|
| `BevelSmoothness`   | `float`             |  For cuboids, the beveled edge smoothness. |
| `LightPosition`     | `Vector3`           | The position, in stage space, of the point light applying lighting to the model. |

-   You can select from 7 `Shape` values, some of which are specializations of another.
...
The following example shows how to use a property map for a primitive visual. The visual is created with the factory `CreateVisual()` method.
