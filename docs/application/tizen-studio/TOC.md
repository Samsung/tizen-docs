# Set up
## Installing
### [Installing Tizen Studio](setup/installing-sdk.md)
### [Prerequisites](setup/prerequisites.md)
### [Updating Tizen Studio](setup/updating-sdk.md)
### [Hardware Accelerated Execution Manager](setup/hardware-accelerated-execution-manager.md)
## [Configuring the Package Manager](setup/advanced-configuration.md)
## [Uninstalling Tizen Studio](setup/uninstalling-sdk.md)

# Native Tools
## [Overview](native-tools/cover-native.md)
## Managing Projects
### [Project Creation Wizard](native-tools/project-wizard.md)
### [Project Explorer View](native-tools/project-explorer-view.md)
### [Certificate Registration](common-tools/certificate-registration.md)
### [Device Manager](common-tools/device-manager.md)
## Writing and Editing Code
### [Content Assist](native-tools/content-assist.md)
### [API and Privilege Checker](native-tools/api-checker.md)
### UI Builder
#### [Overview](native-tools/ui-builder.md)
#### [Storyboard](native-tools/storyboard.md)
#### [Component Attributes](native-tools/component-attributes.md)
#### [Multiple Screen Support](native-tools/multiple-screen.md)
#### [Databinding](native-tools/ui-builder-databind.md)
#### [EFL to UI Builder Project Conversion](native-tools/ui-builder-conversion.md)
#### [Combining Screens from Multiple Applications](native-tools/ui-project-migration-combi.md)
#### [Custom UI Component](native-tools/ui-builder-customcomponent.md)
### [EDC Editor](native-tools/edc-editor.md)
### [Component Designer](native-tools/component-designer.md)
## Configuring Your App
### [Manifest Text Editor](native-tools/manifest-text-editor.md)
### [Additional Manifest Elements](native-tools/manifest-elements.md)
### [Resource Manager](native-tools/resource-manager.md)
### [PO File Localization Editor](native-tools/po-file-editor.md)
## Debugging Your App
### [Log View](common-tools/log-view.md)
### Dynamic Analyzer
#### [Overview](common-tools/da-overview.md)
#### [Common Tasks](common-tools/da-common.md)
#### [Advanced Tasks](common-tools/da-advanced.md)
#### [Memory Analysis](common-tools/da-memory.md)
#### [Thread Analysis](common-tools/da-thread.md)
#### [File Analysis](common-tools/da-file.md)
#### [Network Analysis](common-tools/da-network.md)
#### [OpenGL Analysis](common-tools/da-opengl.md)
#### [UI Hierarchy Analysis](common-tools/da-ui-hierarchy.md)
### [Call Stack View](native-tools/call-stack-view.md)
### [Static Analyzer](native-tools/static-analyzer.md)
### [Valgrind](native-tools/valgrind.md)
### [T-trace](native-tools/t-trace.md)
### [Address Sanitizer](native-tools/address-sanitizer.md)
### [Code Coverage](native-tools/code-coverage.md)
## Running and Testing Your App
### [SDB](common-tools/smart-development-bridge.md)
### Emulator
### [Overview](common-tools/emulator.md)
#### [Emulator Manager](common-tools/emulator-manager.md)
#### [Emulator Control Panel](common-tools/emulator-control-panel.md)
#### [Extended Emulator Features](common-tools/emulator-features.md)
### [Unit Test](native-tools/unit-test.md)
## CLI
### [Overview](common-tools/command-line-interface.md)
### [Converting Projects for CLI](native-tools/project-conversion.md)
## [Keyboard Shortcuts](common-tools/keyboard-shortcuts.md)

# Web Tools
## [Overview](web-tools/cover-web.md)
## Managing Projects
### [Project Creation Wizard](web-tools/project-wizard.md)
### [Project Explorer View](web-tools/project-explorer-view.md)
### [Certificate Registration](common-tools/certificate-registration.md)
### [Device Manager](common-tools/device-manager.md)
## Writing and Editing Code
### [Web Editor](web-tools/web-editor.md)
### [Live Editing](web-tools/live-editing.md)
### [Content Assist](web-tools/content-assist.md)
### [Code Cleaning](web-tools/code-productivity.md)
### [REST Viewer](web-tools/rest-viewer.md)
### [Previewer](web-tools/previewer.md)
### [Privilege Checker](web-tools/privilege-checker.md)
### [Tizen Studio Web Preferences](web-tools/IDE-preferences.md)
## Configuring Your App
### [Configuration Editor](web-tools/config-editor.md)
### [Web Localization](web-tools/web-localization.md)
## Debugging Your App
### [Log View](common-tools/log-view.md)
### Dynamic Analyzer
#### [Overview](common-tools/da-overview.md)
#### [Common Tasks](common-tools/da-common.md)
#### [Advanced Tasks](common-tools/da-advanced.md)
#### [Memory Analysis](common-tools/da-memory.md)
#### [Thread Analysis](common-tools/da-thread.md)
#### [File Analysis](common-tools/da-file.md)
#### [Network Analysis](common-tools/da-network.md)
#### [OpenGL Analysis](common-tools/da-opengl.md)
### [Web Inspector](web-tools/web-inspector.md)
### [JavaScript Analyzer](web-tools/js-analyzer.md)
## Running and Testing Your App
### [SDB](common-tools/smart-development-bridge.md)
### Emulator
#### [Overview](common-tools/emulator.md)
#### [Emulator Manager](common-tools/emulator-manager.md)
#### [Emulator Control Panel](common-tools/emulator-control-panel.md)
#### [Extended Emulator Features](common-tools/emulator-features.md)
### [Web Unit Test Tool](web-tools/web-unit-test-tool.md)
### [Using the Web Simulator](web-tools/web-simulator.md)
### [Web Simulator Features](web-tools/web-simulator-features.md)
## [CLI](common-tools/command-line-interface.md)
## [Keyboard Shortcuts](common-tools/keyboard-shortcuts.md)

# Platform Tools
## [Overview](platform-tools/overview.md)
## [Platform Development Tools](platform-tools/platform-tools.md)

# RT IDE
## Getting Started
### [Overview](rt-ide/getting-started/overview.md)
### [Installing Tizen Studio for RT](rt-ide/getting-started/install.md)
### [Managing a Project](rt-ide/getting-started/create.md)
### [Flashing the Project](rt-ide/getting-started/flash.md)
### [Using the Serial Terminal](rt-ide/getting-started/terminal.md)
### [Debugging the Project](rt-ide/getting-started/debug.md)
## SmartThings&trade; Extension
### [Overview](rt-ide/smartthings-ext/overview.md)
### [Creating a SmartThings&trade; Project](rt-ide/smartthings-ext/create-project.md)
### [Building the SmartThings&trade; Project](rt-ide/smartthings-ext/build-project.md)
### [Managing SmartThings&trade; Certificates](rt-ide/smartthings-ext/manage-certificate.md)
### [Device/Resource Model Manager](rt-ide/smartthings-ext/model-manager.md)

# [Configurable SDK](configurable-sdk/configurable-sdk.md)

# Release Notes
## [2.1 (Jan 3, 2018)](release-notes/2-1-release-notes.md)
## [2.0 (Nov 1, 2017)](release-notes/2-0-release-notes.md)
## [1.3 (Oct 11, 2017)](release-notes/1-3-0-release-notes.md)
## [1.2 (May 10, 2017)](release-notes/1-2-0-release-notes.md)
## [1.1.1 (Feb 27, 2017)](release-notes/1-1-1-release-notes.md)
## [1.1 (Jan 18, 2017)](release-notes/1-1-0-release-notes.md)
## [1.0.2 (Dec 23, 2016)](release-notes/1-0-2-release-notes.md)
## [1.0.1 (Oct 28, 2016)](release-notes/1-0-1-release-notes.md)
## [1.0 (Sep 1, 2016)](release-notes/1-0-release-notes.md)
