...
The Packages configuration allows you to manage the packages of VSCode Extension for Tizen. With the Packages configuration, you can change the options for the package repository for the main and extension SDK.
...
2. Click the **Packages** button. → The Packages page will open.
3. On the Packages page, click the **Repository Setting icon**
   <img src="./media/advanced_conf_browse.png" alt="More Options icon" height="25" style="vertical-align: middle;"/>
...
1. Enter the full path of the SDK image file in the **Package Repository** box, or click next to the combo box to open the file browser.
...
In the Package Manager, developers can select versions and profiles to install necessary packages. When a developer clicks the "Install" button for their desired package, the Package Manager connects directly to the server to download the package. Therefore, if the Host PC being used requires proxy settings due to internal networks at companies, separate proxy configurations are needed.
