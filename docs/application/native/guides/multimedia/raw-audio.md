...
The [Audio Output API](../../api/common/latest/group__CAPI__MEDIA__AUDIO__OUT__MODULE.html) enables your application to play such data using output devices. You can [play audio synchronously](#simple_playback), or [do it asynchronously](#async_playback).
...
The [Audio Input API](../../api/common/latest/group__CAPI__MEDIA__AUDIO__IN__MODULE.html) enables your application to record such data from a microphone type input device. You can [record audio synchronously](#simple_recording), or [do it asynchronously](#async_recording).
...
1. To use the functions and data types of the [Audio I/O](../../api/common/latest/group__CAPI__MEDIA__AUDIO__IO__MODULE.html) and [Sound Manager](../../api/common/latest/group__CAPI__MEDIA__SOUND__MANAGER__MODULE.html) APIs, include the `<audio_io.h>` and `<sound_manager.h>` header files in your application:
...
2. To initialize the audio input and output devices, use the `audio_in_create()` and `audio_out_create_new()` functions:
   ...
   The audio input and output devices support the channel types defined in the [`audio_channel_e` enumeration](../../api/common/latest/group__CAPI__MEDIA__AUDIO__IO__MODULE.html#ga4e07ead99d581a0a049e8ee632b858b4), and the sample types defined in the [`audio_sample_type_e` enumeration](../../api/common/latest/group__CAPI__MEDIA__AUDIO__IO__MODULE.html#ga1e66f976b2890f5fc2e9e6ec71af7536). For playing the recorded audio, use the same channel and sample type on both audio devices.

   The sound types are defined in the [`sound_type_e` enumeration](../../api/common/latest/group__CAPI__MEDIA__SOUND__MANAGER__VOLUME__MODULE.html#gab0b52eeab59765b94c7a751097738a0b). You can select the sound type according to the audio sample type.
...
- To determine the required buffer size based on what is recommended by the Audio I/O API:
  ...
  1. Retrieve the recommended buffer size using the `audio_in_get_buffer_size()` function. The function retrieves the buffer size recommended by the sound server (such as PulseAudio).
     ...
     The recommended buffer size depends on the device. The size can be different for TV, IoT, and other devices.
...
2. Create and open the file for storing the audio data:
   ...
   The `storage_get_directory()` function of the [Storage API](../../api/common/latest/group__CAPI__SYSTEM__STORAGE__MODULE.html) retrieves the storage path based on the storage ID. To retrieve the storage ID, use the `storage_foreach_device_supported()` function of the Storage API.
