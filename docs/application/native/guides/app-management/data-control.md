...
- Monitoring data changes
  ...
  You can [monitor data changes and provide notifications about them](#map3). The available notification types are listed in the `data_control_data_change_type_e` [enumerator](../../api/common/latest/group__CAPI__DATA__CONTROL__MODULE.html#ga09ee00edc0c08676b2fa241f30fab378).
...
1. To use the [Data Control API](../../api/common/latest/group__CAPI__DATA__CONTROL__MODULE.html), the consumer has to request permission by adding the following privileges to the `tizen-manifest.xml` file:
...
2. For the provider, open **tizen-manifest.xml** in the manifest editor and go to **Advanced > Data Control**, and click **+** to add the provider details. Add the **Read** and **Write** access rights to both **SQL** and **Map** types.
...
The provider returns a response to the consumer. The consumer can handle the response in a callback of the `data_control_map_response_cb` [struct](../../api/common/latest/structdata__control__map__response__cb.html), which is triggered when the provider finishes the requested operation.
...
1. Implement the provider application:
   ...
   -  Register the operation callbacks within the `app_create()` function (generated by the project wizard) using the `data_control_provider_map_register_cb()` function:
...
2. Implement the consumer application:
   ...
   -  To identify the provider and data, initialize a data control handler within the `app_create()` function generated by the project wizard:
...
The provider returns a response to the consumer. The consumer can handle the response in a callback of the `data_control_sql_response_cb` [struct](../../api/common/latest/structdata__control__sql__response__cb.html), which is triggered when the provider finishes the requested operation.
...
You can export the provider functionalities of your Tizen native service application in the [application project settings](../../guides/development/setting-properties.md#manifest). The provider ID, type, and accessibility must be specified for the available data control.
