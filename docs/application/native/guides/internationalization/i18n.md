...
The [Ubrk API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UBRK__MODULE.html) is used to [find the location of boundaries in text](#manage_ubrk). The `i18n_ubreak_iterator_h` handle maintains a current position and scans over the text returning the index of characters where the boundaries occur.
...
The [Ucalendar API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCALENDAR__MODULE.html) is used for [converting between a Udate object and a set of integer fields](#dates), such as `I18N_UCALENDAR_YEAR`, `I18N_UCALENDAR_MONTH`, `I18N_UCALENDAR_DAY`, and `I18N_UCALENDAR_HOUR`. A [Udate](#udate) object represents a specific instant in time with one millisecond precision.
...
The [Uchar API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCHAR__MODULE.html) provides low-level access to the Unicode Character Database.
...
The [Udatepg API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UDATEPG__MODULE.html) enables flexible generation of date format patterns, such as "yy-MM-dd". The user can build up the generator by adding successive patterns. After this, a query can be made using a pattern that includes only the desired fields and lengths. The generator returns the a pattern that is most similar to it.
...
The [Uenumeration API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UENUMERATION__MODULE.html) enables you to [create an enumeration object](#uenum) out of a given set of strings. The object can be created out of an array of `const char*` strings or an array of `i18n_uchar*` strings.
...
The [Ulocale API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__ULOCALE__MODULE.html) represents a specific geographical, political, or cultural region. Locale-sensitive operations [use the Ulocale functions to tailor information](#locales) for the user. For example, displaying a number is a locale-sensitive operation. The number must be formatted according to the customs and conventions of the user's native country, region, or culture.
...
The [Unicode normalization API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UNORMALIZATION__MODULE.html) is for the standard Unicode normalization. All instances of `i18n_unormalizer_s` are unmodifiable and immutable. Instances returned by `i18n_unormalization_get_instance()` are singletons that must not be deleted by the caller.

<a name="unumber"></a>
## Number Formats with Unumber

The [Unumber API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UNUMBER__MODULE.html) helps you to [format and parse numbers for any locale](#numbers). Your code can be completely independent of the locale conventions for decimal points, thousands-separators, or even the particular decimal digits used, or whether the number format is even decimal. There are different number format styles like decimal, currency, percent and spellout.

<a name="usearch"></a>
## Text Search with Usearch

The [Usearch API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__USEARCH__MODULE.html) provides language-sensitive text searching based on the comparison rules defined in a Ucollator data struct. This ensures that language eccentricity can be handled. For example, for the German collator, characters ß and SS are matched if case is chosen to be ignored. That is why it can be important to pass a locale when creating the usearch with the `i18n_usearch_create_new()` function.
...
The [Ustring API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__USTRING__MODULE.html) allows you to [handle general Unicode strings](#characters). Some functions are similar in name, signature, and behavior to the ANSI C `<string.h>` functions, and other functions provide more Unicode-specific functionality.
...
The [Alphabetic Index API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__ALPHA__IDX__MODULE.html) allows you to [generate a list of alphabetical labels](#alpha_idx_example) that can be used as an "index", such as in a UI. In other words, it is a list of clickable characters (or character sequences) that allow the user to see a segment (bucket) of a larger target list. Each label corresponds to a bucket in the target list, where all items in the bucket are greater than or equal to the selected character, based on the locale's collation order. Strings added to the index are sorted and placed in order in the appropriate bucket.
...
The [immutable index API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__IMMUTABLE__IDX__MODULE.html) provides an immutable, thread-safe version of an [alphabetic index](#alpha_idx). Immutable indices also allow random access to buckets. An immutable index is constructed using a regular alphabetic index. The immutable index contains similar data as the alphabetic index on which it is based, except for the data records. For more information on the creation and the basic usage of an immutable index, see [example](#immutable_idx_example).

<a name="field_pos"></a>
## Field Identification in Formatted Output

The [FieldPosition API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__FIELD__POSITION__MODULE.html) allows you to [manage the field positions in formatted output](#field_pos_example).
...
The [Format API]](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__FORMAT__MODULE.html) allows you to [manage formatting for displayed strings](#format_examples).
...
The [Formattable API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__FORMATTABLE__MODULE.html) allows you to [interconvert between Udate, char string, and primitive numeric types](#formattable_examples), such as double and long.
...
The [Measure API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MEASURE__MODULE.html) allows you to [create measure objects containing a numerical value and a measurement unit](#measure_examples). You can, for example, store a length value in feet or meters.

The [MeasureUnit API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MEASURE__UNIT__MODULE.html) allows you to [create measure unit objects](#measure_unit_examples) and combine them with numerical values to create measure objects. The [measure objects can also be formatted](#measure_format_examples) using the [MeasureFormat API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MEASURE__FORMAT__MODULE.html).
...
The [ParsePosition API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__PARSE__POSITION__MODULE.html) is used by the [Format API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__FORMAT__MODULE.html) and the [MeasureFormat API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MEASURE__FORMAT__MODULE.html) to [track the current position while parsing](#parse_position_examples).
...
The [Ubidi API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UBIDI__MODULE.html) allows you to [use the Unicode Bidirectional Algorithm](#ubidi_examples) to correctly display text containing characters that flow from right to left, such as Arabic and Hebrew scripts.
...
The [Ushape API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__USHAPE__MODULE.html) allows you to [manage Arabic character shapes](#ushape_examples).
...
The [Utmscale API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UTMSCALE__MODULE.html) allows you to [convert between time scales](#utmscale_examples).
...
The [`i18n_utmscale_scale_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UTMSCALE__MODULE.html#ga1422c14e213019536188c0db3286e438) defines the supported time scales. Their details are listed in the following table.
...
Languages define plural categories in various ways. The [PluralRules API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__PLURAL__RULES__MODULE.html) allows you to [create rules for determining the appropriate plural word form](#plural_rules_examples) when formatting messages using numeric placeholders.
...
The [UCharIter API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCHAR__ITER__MODULE.html) allows you to [iterate through strings](#uchar_iter_examples).
...
The [PluralFormat API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__PLURAL__FORMAT__MODULE.html) allows you to [format defined plural words](#plural_format_examples). Alongside with PluralRules module, you can define rules to which format would apply.
...
The [DateInterval API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__DATE__INTERVAL__MODULE.html) allows you to [create date intervals](#date_interval_examples).
...
The [DateIntervalFormat API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__DATE__INTERVAL__FORMAT__MODULE.html) allows you to [format created date intervals](#date_interval_examples).
...
For the non-digit calendar fields, the pattern letter length is important. The pattern length are MMM, MMMM, and MMMMM; EEE and EEEE.
...
For other calendar fields, the compact interval formatting is not supported. The function returns formatted date intervals depending on the interval value.
The date interval format is {date0} - {date1}. For example, 20 - 25 September or 20 April - 15 May.
...
To use the functions and data types of the [i18n API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MODULE.html) and its submodules, include the `<utils_i18n.h>` header file in your application:
...
For a more complex, locale-sensitive comparison, use the [Ucollator API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCOLLATOR__MODULE.html):
...
The function returns, as an out parameter, the Unicode allocation block that contains the specified character. The Unicode blocks are defined in the [`i18n_uchar_ublock_code_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCHAR__MODULE.html#gaa338daff96b0e62243e25fe240e4eda5).
...
The character properties are defined in the [`i18n_uchar_uproperty_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCHAR__MODULE.html#gaa57de8e60ee941839fdfd80833106757).
...
- Use the `i18n_uchar_has_binary_property()` function with the name of the binary property.
  ...
  The binary properties are defined in the [`i18n_uchar_uproperty_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCHAR__MODULE.html#gaa57de8e60ee941839fdfd80833106757).
...
1. Get a Unormalizer instance using the `i18n_unormalization_get_instance()` function of the [Unormalization API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UNORMALIZATION__MODULE.html).
...
1. Create a search iterator using the `i18n_usearch_create_new()` function of the [Usearch API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__USEARCH__MODULE.html):
...
1. To manage dates and calendar, the [Ucalendar](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCALENDAR__MODULE.html), [Udate](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UDATE__MODULE.html), and [Udatepg](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UDATEPG__MODULE.html) APIs are used.
...
3. To set a date in the Ucalendar, use the `i18n_ucalendar_set_date_time()` function.
    ...
    To define the month, you can use numbers (month reduced by 1, such as 0 for January and 1 for February), or to avoid mistakes, the values of the [`i18n_ucalendar_months_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCALENDAR__MODULE.html#ga094cacb2ef9ee4805e42e276fec5ae2f).
    ...
    Specify the date field to modify (such as year, week, or day) using the [`i18n_ucalendar_date_fields_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCALENDAR__MODULE.html#gaee345f9992035a07732d16d69c41c192) as the second parameter, and the amount to modify as the third parameter (use a negative value to subtract from the existing value).
...
5. To format a date, you can use a pattern generator or define a date format:
   ...
   - Using a date format:
     1. To create a date format, use the `i18n_udate_create()` function.
        ...
        As the first and second parameter, specify the formatting style for time and date using the values of the [`i18n_udate_format_style_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UDATE__MODULE.html#gaee2461e926bc151486d380c43bc4f2a3).
...
1. To create a number format, use the `i18n_unumber_create()` function.
    ...
    Define the style as the first parameter using the [`i18n_unumber_format_style_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UNUMBER__MODULE.html#ga4edc8cb72e7f46e05d8cdfe24cf386f1).
...
3. To get a symbol associated with the created number format, use the `i18n_unumber_get_symbol()` function.
    ...
    The second parameter uses the values of the [`i18n_unumber_format_symbol_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UNUMBER__MODULE.html#ga9abb496f12b9fd47244060af5ecbc39e) to define the symbol to be retrieved.
...
Since Tizen 3.0, you can generate an alphabetical list of clickable labels by using the [Alphabetic Index API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__ALPHA__IDX__MODULE.html).
...
1. To create an alphabetic index object for a specific locale:
   ...
   > **Note**
   >
   > This function is deprecated from Tizen 5.0. Instead, use `i18n_alpha_idx_create_from_locale_id()`,
   > which takes the locale ID parameter. To set the language as "en" and country as "US", use the locale ID "en_US".
...
2. To manage index labels:
   ...
   - To add labels from a specific locale:
     ...
     > **Note**
     >
     > This function is deprecated from Tizen 5.0. Instead, use `i18n_alpha_idx_add_labels_with_locale_id()`,
     > which takes the locale ID parameter. To set the language as "ko" and country as "KR", use the locale ID "ko_KR".
...
From Tizen 5.0, you can use [immutable index API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__IMMUTABLE__IDX__MODULE.html) to create an immutable index, a thread-safe version of an [alphabetic index](#alpha_idx) without data records.
...
Since Tizen 3.0, you can use the [FieldPosition API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__FIELD__POSITION__MODULE.html) to manage the field position for formatted output.
...
Since Tizen 3.0, you can use the [Format API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__FORMAT__MODULE.html) to manage format objects, which derive from this module, and their string representations:
...
2. To retrieve the locale from a format object:
   ...
   > **Note**
   >
   > This function is deprecated from Tizen 5.0. Instead, use `i18n_format_get_locale_id()`,
   > which will return the locale ID parameter. For example for language "en" and country "US", this function
   > will return "en_US" string.
...
You can use the [Formattable API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__FORMATTABLE__MODULE.html) to convert between data types.
...
Since Tizen 3.0, you can use the [Measure API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MEASURE__MODULE.html) to create measure objects, containing a numerical value and a measurement unit.
...
You can use the [MeasureUnit API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MEASURE__UNIT__MODULE.html) to create measure unit objects, which can be passed to the [Measure or MeasureFormat](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MEASURE__FORMAT__MODULE.html) APIs.
...
You can format measure objects using the [MeasureFormat API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MEASURE__FORMAT__MODULE.html).
...
1. To create a format object:
   ...
   > **Note**
   >
   > This function is deprecated from Tizen 5.0. Instead, use `i18n_measure_format_create_from_locale_id()`,
   > which takes the locale ID parameter. To set the language as "en" and country as "US", use the locale ID "en_US".
...
5. To retrieve the currency formatter:
   ...
   - For a specific locale:
     ...
     > **Note**
     >
     > This function is deprecated from Tizen 5.0. Instead, use `i18n_measure_format_create_currency_format_from_locale_id()`,
     > which takes the locale ID parameter. To set the language as "ko" and country as "KR", use the locale ID "ko_KR".
...
Since Tizen 3.0, you can use the [ParsePosition API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__PARSE__POSITION__MODULE.html) to track the current position while parsing.
...
Since Tizen 3.0, you can use the [Ubidi API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UBIDI__MODULE.html) to implement the Unicode Bidirectional Algorithm.
...
Since Tizen 3.0, you can use the [Ushape API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__USHAPE__MODULE.html) to manage Arabic character shapes.
...
Since Tizen 3.0, you can use the [Utmscale API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UTMSCALE__MODULE.html) to convert binary datetimes between various platform-dependent time scales.

To convert a datetime value:

1. To retrieve conversion constants for a specific time scale, use the `i18n_utmscale_get_time_scale_value()` function with the values of the [`i18n_utmscale_value_e` enumeration](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UTMSCALE__MODULE.html#ga34893d20b446359e32767b57e6cdde29):
...
Since Tizen 4.0, you can use the [PluralRules API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__PLURAL__RULES__MODULE.html) to create rules for number-dependent word representations.
...
3. To retrieve predefined rules for a specific locale:
   ...
   > **Note**
   >
   > This function is deprecated from Tizen 5.0. Instead, use `i18n_plural_rules_for_locale_id()`,
   > which takes the locale ID parameter. To set the language as "en" and country as "US", use the locale ID "en_US".
...
Since Tizen 4.0, you can retrieve the current ICU library version by using the [Uversion API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UVERSION__MODULE.html).
...
Since Tizen 4.0, you can use [UcharIter API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__UCHAR__ITER__MODULE.html) to safely iterate through strings:
...
From Tizen 5.0, you can use the [PluralFormat API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__PLURAL__FORMAT__MODULE.html) to format words for its number-dependent representations.
...
<a name="date_interval_examples"></a>
## Manage Date Intervals

From Tizen 5.0, you can use the [DateInterval API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__DATE__INTERVAL__MODULE.html) and
the [DateIntervalFormat API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__DATE__INTERVAL__FORMAT__MODULE.html) to manage and format date intervals.
