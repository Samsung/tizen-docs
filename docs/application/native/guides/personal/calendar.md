...
- Basic types
  ...
  **Table: Setter and getter functions**
  ...
  The time structure can hold 2 types of data, as defined in the following table. These types are identified by the values of the [`calendar_time_type_e` variable](../../api/common/latest/group__CAPI__SOCIAL__CALENDAR__SVC__RECORD__MODULE.html#ga809c1bb1db41169c65939b929520e9b6), and they determine the usage of the structure.
  ...
  **Table: Data types**
  ...
  When converting local time to UTC time, use the function shown in the following example. It converts the given date and time to the corresponding UTC time, considering the given time zone (first parameter). The function uses the [i18n API](../../api/common/latest/group__CAPI__BASE__UTILS__I18N__MODULE.html).
...
1. To use the [Calendar API](../../api/common/latest/group__CAPI__SOCIAL__CALENDAR__SVC__MODULE.html), the application has to request permission by adding the following privileges to the `tizen-manifest.xml` file:
...
Some event properties are defined as child records that are associated with the parent record. For a detailed list of the event properties, see the `_calendar_event` view description in the [Calendar API](../../api/common/latest/group__CAPI__SOCIAL__CALENDAR__SVC__VIEW__MODULE.html#CAPI_SOCIAL_CALENDAR_SVC_VIEW_MODULE_calendar_event). If the property type is `child list`, the property is defined as a child record.
...
2. Set the event properties:
   ...
   - Set the start and end times.
     ...
     The [`calendar_time_s` structure](../../api/common/latest/structcalendar__time__s.html) has [2 types](#time).

   - To create a recurring event:

     1. Set the [frequency](../../api/common/latest/group__CAPI__SOCIAL__CALENDAR__SVC__MODULE.html#CAPI_SOCIAL_CALENDAR_SVC_MODULE_Creating_a_recurring_event).
     ...
     2. Set the [range of recurrence](../../api/common/latest/group__CAPI__SOCIAL__CALENDAR__SVC__MODULE.html#CAPI_SOCIAL_CALENDAR_SVC_MODULE_Creating_a_recurring_event).
   ...
   - Add an alarm.
     ...
     The [`calendar_alarm_time_unit_type_e` enumeration](../../api/common/latest/group__CAPI__SOCIAL__CALENDAR__SVC__RECORD__MODULE.html#ga631b1b606158479e3a14a921b006b4fe) defines the available alarm tick units.
...
1. Retrieve a list of all events, or retrieve a filtered list of events:
   ...
   - To retrieve a filtered list of events:
     ...
     4. To add more conditions, define an operator between the conditions.
        ...
        The [`type` parameter of the `calendar_time_s` structure](../../api/common/latest/structcalendar__time__s.html) determines whether the event is an all-day event (`CALENDAR_TIME_LOCALTIME`) or a non-all-day event (`CALENDAR_TIME_UTIME`).
...
Some todo properties are defined as child records that are associated with the parent record. For a detailed list of the todo properties, see the `_calendar_todo` view description in the [Calendar API](../../api/common/latest/group__CAPI__SOCIAL__CALENDAR__SVC__VIEW__MODULE.html#CAPI_SOCIAL_CALENDAR_SVC_VIEW_MODULE_calendar_todo). If the property type is `child list`, the property is defined as a child record.
...
2. Set the todo properties:
   ...
   - Set the status.
     ...
     To set the status for the todo, use the `calendar_record_set_int()` function with the `_calendar_todo.todo_status` property as the second parameter. The [`calendar_todo_status_e` enumeration](../../api/common/latest/group__CAPI__SOCIAL__CALENDAR__SVC__RECORD__MODULE.html#ga3e1b9cae05705d471a4746d8ab6d3622) defines the possible status values.
