...
- URI
  ...
  For a list of functions that need the `_uri` postfix, see the list of functions in the [Contacts API](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__MODULE.html#CAPI_SOCIAL_CONTACTS_SVC_MODULE_RECORDS_URI).
...
- Views
  ...
  **Figure: Properties**
  ...
  For more information, see the [View/Property API](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__VIEW__MODULE.html).
...
- Filtering
  ...
  The [Filter API](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__FILTER__MODULE.html) provides the set of definitions and interfaces that enable you to make filters to set queries.
...
To detect the [person](#monitor_contact) and [group](#monitor) changes in the contacts database, use the [Database API functions](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__DATABASE__MODULE.html). Register a callback with the `contacts_db_add_changed_cb()` function. To deregister the callback and ignore database changes, use the `contacts_db_remove_changed_cb()` function.
...
1. To use the [Contacts API](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__MODULE.html), the application has to request permission by adding the following privileges to the `tizen-manifest.xml` file:
...
Some contact properties are defined as child records that are associated with the parent record. For a detailed list of the contact properties, see the `_contacts_contact` view description in the [Contacts API](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__VIEW__MODULE.html#CAPI_SOCIAL_CONTACTS_SVC_VIEW_MODULE_contacts_contact). If the property type is `record`, the property is defined as a child record. The property description defines whether a single or multiple child records are allowed for a specific property.
...
2. Set the contact properties:
   ...
   - To set an event for the contact:
     ...
     An event consists of an event type, date, and other properties. You can set various types of events for the contact, as defined in the [`contacts_event_type_e` enumeration](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__RECORD__MODULE.html#ga434cc4b7cec62ccab70fa4825ce0801d). If the event type is `CONTACTS_EVENT_TYPE_CUSTOM`, you can set a custom label for the event using the `contacts_record_set_str()` function with the `_contacts_event.label` property as the second parameter.
...
- Retrieve more record details using a structure:
  ...
  2. Retrieve record data into the structure.
     ...
     For a list of properties by view, see [View/Property](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__VIEW__MODULE.html).
     ...
     - To retrieve the contact's phone number, first check whether the contact has at least 1 phone number using the `contacts_record_get_bool()` function on the contact record with the `_contacts_contact.has_phonenumber` property. If the contact has 1 or more phone numbers, retrieve the numbers using the various [Query](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__QUERY__MODULE.html) and [Filter](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__FILTER__MODULE.html) functions from the Contacts API. The numbers are stored in the number child records (the `_contacts_number` view). After you are done, free any data returned by a function not containing the `_p` suffix.
...
1. Retrieve a list of records:
   ...
   - To retrieve a filtered and sorted list of records of a given type:
     ...
     2. Create a filter for the query using the `contacts_filter_create()` function:
        ...
        The first parameter defines in which view to place the filter. To filter contacts by first and last name, use the `_contacts_name` view. The first parameter must be the same as the first parameter of the `contacts_query_create()` function, that is, the query and its filter must both use the same view. For more information on views, see [View/Property](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__VIEW__MODULE.html).
     ...
     3. Add a filtering condition using a `contacts_filter_add_XXX()` function.
        ...
        The available matching options (third parameter) for string-based filtering conditions are defined in the [`contacts_match_str_flag_e` enumeration](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__FILTER__MODULE.html#ga5a4ee5c71ae14d0fbf7520597514f0c2).
...
- Set the default properties for the linked person.
  ...
  Set the default properties from one of the associated contacts using the `contacts_person_set_default_property()` function. The first parameter uses the values of the [`contacts_person_property_e` enumeration](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__PERSON__MODULE.html#ga641465951ce76daa56bb430b37cc8d90), which defines the available default properties for a person.
...
3. Change the display and sorting orders using the `contacts_setting_set_name_display_order()` and `contacts_setting_set_name_sorting_order()` functions. Both functions use as a parameter a value of an enumeration: [`contacts_name_display_order_e`](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__SETTING__MODULE.html#ga0b52839d82e7ca998436b174e1f807d8) and [`contacts_name_sorting_order_e`](../../api/common/latest/group__CAPI__SOCIAL__CONTACTS__SVC__SETTING__MODULE.html#ga28fdacd75efe1b14b33a3215fc5c3854) that define the available display and sorting orders.
