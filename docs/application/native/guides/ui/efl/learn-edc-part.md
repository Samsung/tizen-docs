  ...
  - `description.box` block
    ...
    - `layout [primary layout] (fallback layout)`
      ...
      - `some_other_custom_layout_set_by_the_application`
        ...
        Set a custom layout as a fallback. For more information, see the `edje_box_layout_register()` function in the [Edje Box Part API](../../../api/common/latest/group__Edje__Part__Box.html). If an unregistered layout is used, it defaults to `horizontal`.
