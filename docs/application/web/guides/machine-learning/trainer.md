...
> [!NOTE]
> For clarity, errors in the examples are not handled correctly. In production code, take care of all of them to prevent your application from terminating. All errors can be found in [API reference](../../api/latest/device_api/tv/tizen/ml_trainer.html).
...
Optionally you can [set properties](../../api/latest/device_api/tv/tizen/ml_trainer.html#Dataset::setProperty) of the dataset with `setProperty` method:
...
The trained model can be saved to a file for future use. Let's use the `FORMAT_INI_WITH_BIN` format, which will produce two files: an `ini` file with the network structure and a binary file with the weights. There are more formats which can be found in [API reference](../../api/latest/device_api/tv/tizen/ml_trainer.html#Model::saveToFile). Remember to add proper privileges to the `config.xml` as described in [the prerequisites](#prerequisites):
