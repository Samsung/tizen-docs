...
5. If the device has been connected call `getAttMtu()` function on the device object to get current ATT MTU value:
   ...
   ```
6. If the device has been connected call `requestAttMtuChange()` function on the device object to request cahnge of the ATT MTU value. Pass desired ATT MTU value as an attribute:
   ```
