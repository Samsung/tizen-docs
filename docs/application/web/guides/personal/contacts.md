...
You can manage the contacts and persons listed in your address books. A `Contact` object is always associated with a specific address book. A `Person` object is an aggregation of 1 or more contacts associated with the same person.
...
1. To make your application visible in the official site for Tizen applications only for devices that support the contact feature, the application must specify the following feature in the `config.xml` file:
   ...
   Additionally, to double-check the Contact API support while the application is running, use the `tizen.systeminfo.getCapability()` method and enable or disable the code that needs the API, as needed.
...
2. To create a `ContactGroup` object and add the newly create group to the system, use the constructor and the `addGroup()` method of the `ContactManager` interface:
...
To convert multiple strings and import them to an address book, convert the strings one by one and then use the `addBatch()` method of the `AddressBook` interface to [add all the contacts at once in the batch mode](#adding_multiple_contact).
