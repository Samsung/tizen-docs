...
1. To make your application visible in the official site for Tizen applications only for devices that support the calendar feature, the application must specify the following feature in the `config.xml` file:
   ...
   Additionally, to double-check the Calendar API support while the application is running, use the `tizen.systeminfo.getCapability()` method and enable or disable the code that needs the API, as needed.
...
2. Define a success callback for the `getAccounts()` method. The callback receives a list of `Account` objects. Use the first account ID to construct a new `Calendar` object.  
...
Using the `CalendarEvent` object, you can do the following:
...
Using the `CalendarTask` object, you can do the following:
...
4. Add the `CalendarTask` object to the default calendar with the `add()` method of the `Calendar` object:
...
3. Use the `addBatch()` method of the `Calendar` object to add the tasks in the array to the calendar:
