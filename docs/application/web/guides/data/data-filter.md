...
The Tizen API is mandatory for Tizen TV, IoT, and other profiles, which means that it is supported on all TV, IoT, and other devices. All mandatory APIs are supported on the Tizen emulators.

The main data handling features of the Tizen API include the following:

- Filters

  The Tizen API supports the following filter types, which are subtypes of the `AbstractFilter` interface (in [TV](../../api/latest/device_api/tv/tizen/tizen.html#AbstractFilter) applications):

  - `AttributeFilter` (in [TV](../../api/latest/device_api/tv/tizen/tizen.html#AttributeFilter) applications)  
  ...
  - `AttributeRangeFilter`(in [TV](../../api/latest/device_api/tv/tizen/tizen.html#AttributeRangeFilter) applications)  
...
1. The `AttributeFilter` filter (in [TV](../../api/latest/device_api/tv/tizen/tizen.html#AttributeFilter) applications) is used to filter the search results based on an attribute value. In this example, the filter finds contacts, with the first name Chris, from the default address book.
...
2. Call the `find()` method of the `AddressBook` interface (Optional feature) to find contacts. The filter (`firstNameFilter`) you created is included as a parameter:
...
1. The `AttributeRangeFilter` filter (in [TV](../../api/latest/device_api/tv/tizen/tizen.html#AttributeRangeFilter) applications)is used to search for results based on a range of attribute values. In this example, the filter finds all events starting on a defined day from the calendar.
...
2.  Call the `find()` method of the `Calendar` interface (Optional feature) to find events. The filter (`dateRangeFilter`) you created is included as a parameter:
...
1. The `CompositeFilter` filter (in [TV](../../api/latest/device_api/tv/tizen/tizen.html#CompositeFilter) applications) is used to search for results based on a set of filters. In this example, the filter finds contacts, whose first name contains Chris and last name is Smith, from the default address book.
...
2.  Call the `find()` method of the `AddressBook` interface (Optional feature) to find matching contacts. The filter (`nameCompositeFilter`) you created is included as a parameter:
...
1. The `SortMode` interface (in [TV](../../api/latest/device_api/tv/tizen/tizen.html#SortMode) applications) is created to sort the search results. In this example, it is used to sort contacts in the device address book in ascending order by first name.
...
2.  Call the `find()` method of the `AddressBook` interface (Optional feature) to find matching contacts. In this example, the filter parameter in the `find()` method is defined as `null`, which means that the method retrieves all contacts in the address book:
...
> For example, if the `ContentType` enumeration (in [TV](../../api/latest/device_api/tv/tizen/content.html#ContentType) applications) is the type attribute and the sorting order is `"DESC"`, the sorting result is ordered according to the listed enumerator order:
> - Image
> - Video
> - Audio
> - Other
...
5. Call the `find()` method of the Content API's `ContentManager` interface (in [TV](../../api/latest/device_api/tv/tizen/content.html#ContentManager) applications) to retrieve the media items that match the final filter:
...
The following table lists the filter types you can use with specific calendar item attributes in the methods of the `Calendar` interface (Optional feature).
...
The following table lists the filter types you can use with specific call history attributes in the methods of the CallHistory interface (Optional feature).
...
The following table lists the filter types you can use with specific contact attributes in the methods of the `AddressBook` interface (Optional feature).
...
The following table lists the filter types you can use with specific person attributes in the methods of the `ContactManager` interface (Optional feature).
...
The following table lists the filter types you can use with specific content attributes in the methods of the `ContentManager` interface (in [TV](../../api/latest/device_api/tv/tizen/content.html#ContentManager) applications).
...
> [!NOTE]
> The `FULLSTRING` value of the `FilterMatchFlag` enumerator (in [TV](../../api/latest/device_api/tv/tizen/tizen.html#FilterMatchFlag) applications) is not supported and returns an error callback.
