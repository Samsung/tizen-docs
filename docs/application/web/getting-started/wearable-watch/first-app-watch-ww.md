...
-   On a low-powered wearable device, an ambient mode is available. In this mode, the watch application shows a limited UI and receives only the ambient tick event every minute to reduce power consumption.
    ...
    If the application contains a custom UI for the [ambient mode](../../tutorials/details/event-handling-w.md#ambient), the ambient mode support is also shown in the `config.xml` file. However, this sample application does not support a custom ambient UI.
...
You can run the Web watch application on the [emulator](../../tutorials/process/run-debug-app-w.md#emulator) or a [real target device](../../tutorials/process/run-debug-app-w.md#target).
