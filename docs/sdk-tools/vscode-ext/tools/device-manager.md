...
The **Device Manager** is part of the extension that manages devices and allows you to access logs. A device can be either a virtual device, such as an emulator, or a physical device connected via a USB cable or a wireless connection.
...
There are two different ways to access the Device Manager:

- Click **Device** in the **Active Targets** section.

  ![Open Device Manager from Active Target](media/device_active_target.png)

- Open **Device Manager** from the **TIZEN** panel.
...
- **Remote device**
  ...
  1. Open the **TIZEN** panel and click the **REMOTE DEVICES** tab.
  ...
  4. In the **Add Device** dialog that appears, enter the device name, the IP address of the device to which the connection needs to be made, and the port number. The default port number is **26101**.
     ...
     > [!NOTE]
     > If you are using an emulator, you need to enter the IP address of the system on which the emulator is launched.

  5. Confirm the IP address and port number of the remote device, and click the **Connect** button to connect to the remote device.
...
When a device is connected to the Device Manager, a device log tab can be opened to view logs. You can set different filters for each log tab. Tabs for disconnected devices are shown as inactive and are reactivated when the device is reconnected.

- Click the ![Device Log Icon](./media/device_log_icon.png) icon from the device row for which you want to view the log. It will automatically open the log tab.
...
- **Filtering log messages**
  ...
  - To filter log messages based on the **Level**, click the ![Down arrow](./media/device_manager_downarrow_icon.png) icon next to the column name and select the value you want to show.
  - To filter log messages based on the **PID**, **TID**, **Tag**, and **Message** columns, enter the string you want to search for in the field at the top of the **Log** view.
